...
void SwerveModule::OnUpdate(units::second_t dt) {
  ...
  switch (_state) {
    ...
    case SwerveModuleState::kPID: {
      ...
      // std::cout << "GetEncoderPosition() : " << _config.turnMotor.encoder->GetEncoderPosition().value() << std::endl;
      // std::cout << "ANGLE Setpoint: " << _anglePIDController.GetSetpoint().value() << std::endl;
      // std::cout << "DRIVE Setpoint: " << _velocityPIDController.GetSetpoint().value() << std::endl;
      if (GetSpeed().value() == NAN) {
        std::cout << "GET SPEED IS NAN" << std::endl;
        driveVoltage =
          _velocityPIDController.Calculate(0_mps, dt, 0.05_V);
      } else {
        driveVoltage =
  ...
  // std::cout << "Turn Set Point : " << _anglePIDController.GetSetpoint().value() << std::endl;
  // _config.driveMotor.motorController->SetVoltage(2_V);
  // _config.turnMotor.motorController->SetVoltage(2_V);
...
void SwerveDrive::OnStart() {
  std::cout << "Swerve Drivebase On Start" << std::endl;
